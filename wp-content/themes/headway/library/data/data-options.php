...
class HeadwayOption {
	...
	public static function get($option = null, $group_name = false, $default = null, $main_site = false) {
		...
		public static function get_group($group_name, $main_site = false) {
			...
			/* Query for the option group */
				if ( !$main_site || !is_multisite() )
					...
					$group_data = get_blog_option($current_site->blog_id, 'headway_option_group_' . $group_name . self::$group_suffix);

			/* If a group suffix is being used and no data exists for that option with suffix then pull normal option */
				if ( self::$group_suffix && !$group_data ) {

					if ( !$main_site || !is_multisite() )
						$group_data = get_option('headway_option_group_' . $group_name);
					else
						$group_data = get_blog_option($current_site->blog_id, 'headway_option_group_' . $group_name);
		...
		public static function get_from_main_site($option = null, $group_name = false, $default = null) {
			...
			return self::get($option, $group_name, $default, true);
